...
function findEvent ({event, path}, events) {
  for (let i = events.length - 1; i >= 0; i--) {
    let item = events[i];
    if (path === item.path && event === item.event) {
      return i;
    }
  }
...
module.exports = config => {
  return app => {
    ...
    const enableSkipEmitEvent = config.enableSkipEmitEvent || true;
    const enabledPathEventsMap = config.enabledPathEventsMap || {};
    ...
    app.on('sync-out', data => {
      ...
      let i = (enabledPathEventsMap[data.path] || []).indexOf(data.event);
      if (i < 0 || (enableSkipEmitEvent && (data.data || {}).skipEmitEvent === true)) {
        return;
      }

      if (data.data.skipSpreadEventToOthers === true) {
    ...
    sub.on('message', function (e, data) {
      if (e === key) {
        ...
        let i = findEvent(obj, mergingEvents);
        if (i >= 0) {
